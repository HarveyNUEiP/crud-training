...
class Test extends CI_Controller
{
    ...
    protected function _list($arr_params)
    {
        ...
        $this->load->model('test_model');
        // 取得資料
        $res = $this->test_model->get($arr_params);
        // 取得資料筆數
        $num = $this->test_model->getNumbers($arr_params['keywords']);
        // 建立輸出陣列
        $opt = [
            'numbers' => $num,
            'data' => $res
    ...
    protected function _update($id, $data)
    {
        try {
            ...
            $this->load->model('test_model');
            // 資料更新至資料庫
            $opt = $this->test_model->put($id, $data);
            // 輸出JSON
            echo json_encode($opt);
        } catch (Exception $e) {
            // 印出錯誤代碼
    ...
    public function export()
    {
        ...
        $this->load->model('test_model');
        // 取得資料
        $data = $this->test_model->get($params);
    ...
    public function import()
    {
        ...
        try {
            ...
            $this->load->model('test_model');
            // 取得資料已存在之帳號
            $arr_exist = $this->test_model->getAcc($acc_arr, 'account');
            // 已存在帳號陣列處理
            ...
            if (!empty($insert_data)) {
                // 批次新增
                $this->test_model->batchAdd($insert_data);
            }
            if (!empty($update_data)) {
                // 批次修改
                $this->test_model->batchUpdate($update_data);
            }
        } catch (Exception $e) {
